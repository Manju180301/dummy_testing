...
class Rule(models.Model):
    RULE_TYPES = [
        ...
        ("no_employee", "No Employee Rule"),
        ("group", "Group Detection Rule"),  
    ]
...
class CameraBoundary(models.Model):
    camera_name = models.CharField(max_length=100)
    boundary_name = models.CharField(max_length=100)
    points = models.JSONField()  # list of [[x1,y1],[x2,y2],[x3,y3],[x4,y4]]
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"{self.camera_name}: {self.boundary_name}"
    
class NoEmployeeShift(models.Model):
